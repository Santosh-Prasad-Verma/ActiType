...
async function queryRequest(text, isMCQ = false, isMultipleChoice = false, tabId = null) {
    ...
    try {
        ...
        if (customAPIConfig.useCustomAPI && customAPIConfig.apiKey) {
            ...
            if (tabId) {
                showToast(tabId, 'Please configure your API key in Settings.', true, 'You must provide your own API keys in the Settings tab.');
            }
            ...
            return { 
                error: 'Please configure your custom API key in Settings.', 
                errorType: 'auth',
                detailedInfo: 'You must provide your own API keys in the Settings tab to use this extension.'
            };
        }
    } catch (error) {
...
// Removed API_BASE_URL and getTokens for custom API configuration
...
async function handleChatMessage(message, sender) {
    try {
        ...
        if (customAPIConfig.useCustomAPI && customAPIConfig.apiKey) {
            ...
            sendChatErrorResponse(sender.tab.id, "Please configure your custom API key in Settings to use the chat.");
        }
    } catch (error) {
        console.error("Error in handleChatMessage:", error);
        sendChatErrorResponse(sender.tab.id, "An unexpected error occurred.");
    }
}
...
// Remote logout handling removed for Custom API
...
// Session expiration handling removed for Custom API
